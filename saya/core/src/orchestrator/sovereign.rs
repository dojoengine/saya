...
pub struct SovereignOrchestrator<I, P, D, S> {
    cursor_channel: Receiver<DataAvailabilityCursor>,
    ingestor: I,
    prover: P,
    da: D,
    storage: S,
...
where
    ...
    P: ProverBuilder + Send,
    D: DataAvailabilityBackendBuilder + Send,
    S: StorageBackend,
{
    pub async fn build(
        self,
        ...
        let (proof_tx, proof_rx) = tokio::sync::mpsc::channel::<Proof>(PROOF_BUFFER_SIZE);
        let (cursor_tx, cursor_rx) =
            tokio::sync::mpsc::channel::<DataAvailabilityCursor>(CURSOR_BUFFER_SIZE);
        ...
        loop {
            ...
            self.storage
                .set_chain_head(BlockWithDa {
                    height: new_cursor.to_block_number,
                    da_pointer: new_cursor.pointer,
                })
                .await;

            info!("Chain advanced to block #{}", new_cursor.to_block_number);
        }
