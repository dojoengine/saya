...
//! ```
//!
//! Because the ingestor dispatches blocks sequentially, batches are always ordered — no
//! `BlockOrderer` is needed between the ingestor and the attestor.  A `BlockOrderer` will be
//! added after the prover when parallel proving is introduced.
...
/// Size of the `Vec<BlockInfo>` channel between ingestor and the attestor.
const BLOCK_INGESTOR_BUFFER_SIZE: usize = 4;

/// Size of the `TeeAttestation` channel between attestor and prover.
const ATTESTATION_BUFFER_SIZE: usize = 4;

/// Size of the `TeeProof` channel between prover and settlement.
