...
impl TeeAttestation {
    ...
    /// Generate SP1 Groth16 proof for this attestation (attestation only, no storage proof).
    ...
    ///
    /// When `storage` is `Some`, the ZK circuit verifies the storage proof and commits
    /// `keccak256(abi.encode(keys, values))` to the journal (contract can then check commitment).
    /// When `event_proof` is `Some`, the ZK circuit verifies event inclusion (C2: shard ending).
    ///
    /// Architecture: we spawn a dedicated OS thread because `reqwest::blocking` inside KDS
    /// panics if called from within a tokio runtime. The thread runs two phases:
    /// 1. KDS cert fetch (blocking HTTP, no tokio)
    /// 2. Registry lookup + SP1 proof generation (new single-thread tokio runtime)
    pub async fn generate_proof_with_storage(
        ...
        let handle = std::thread::spawn(move || -> Result<OnchainProof> {
            // Phase 1: KDS cert fetch — must run outside tokio (reqwest::blocking panics in block_on).
        ...
        // Wait for the proof thread with a timeout. On timeout the thread is detached
        // (acceptable for a CLI — the process will exit).
